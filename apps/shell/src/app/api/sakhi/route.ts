...
export const maxDuration = 120; // allow 2 min for local Ollama on CPU
...
export async function POST(req: NextRequest) {
  try {
    ...
    // LLM fallback for unmatched questions (requires ANTHROPIC_API_KEY)
    try {
      const answer = await llmAnswer(question);
      return Response.json({ answer });
    } catch {
      return Response.json({ answer: OUT_OF_SCOPE });
    }
  } catch {
